...
impl RdtcBox {
    pub fn read(data: &[u8]) -> RdtcBox {
        ...
        RdtcBox { base, data_table }
    }
}

impl Serialize for RdtcBox {
...
mod tests {
    ...
    fn test_rdtcbox_read() {
        ...
        let rdtc_box = RdtcBox::read(&data);
        assert_eq!(rdtc_box.data_table.len(), 2);

        assert_eq!(rdtc_box.data_table[0].x, 10.0);
        assert_eq!(rdtc_box.data_table[0].y, 12.0);
        assert_eq!(rdtc_box.data_table[0].z, 14.0);
        assert_eq!(rdtc_box.data_table[0].timestamp, 1);

        assert_eq!(rdtc_box.data_table[1].x, 20.0);
        assert_eq!(rdtc_box.data_table[1].y, 22.0);
        assert_eq!(rdtc_box.data_table[1].z, 24.0);
        assert_eq!(rdtc_box.data_table[1].timestamp, 2);
    }
    
    #[test]
    fn test_rdtc_box_to_json() {
        let data: Vec<u8> = setup();
        let rdtc_box = RdtcBox::read(&data);
        let json_output = serde_json::to_string_pretty(&rdtc_box).unwrap();
