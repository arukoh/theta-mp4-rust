...
impl RdtaBox {
    pub fn read(data: &[u8]) -> RdtaBox {
        ...
        for i in 0..number_of_entries as usize {
            ...
            let (x, y, z, reserve, timestamp) = match base.endian {
                0x0123 => (
                    ...
                    f32::from_le_bytes(data[entry_offset + 4..entry_offset + 8].try_into().unwrap()), // y
                    f32::from_le_bytes(data[entry_offset + 8..entry_offset + 12].try_into().unwrap()), // z
                    f32::from_le_bytes(data[entry_offset + 12..entry_offset + 16].try_into().unwrap()), // reserve
                    u64::from_le_bytes(data[entry_offset + 16..entry_offset + 24].try_into().unwrap()), // timestamp
                ),
                0x3210 => (
                    f32::from_be_bytes(data[entry_offset..entry_offset + 4].try_into().unwrap()), // x
                    f32::from_be_bytes(data[entry_offset + 4..entry_offset + 8].try_into().unwrap()), // y
                    f32::from_be_bytes(data[entry_offset + 8..entry_offset + 12].try_into().unwrap()), // z
                    f32::from_be_bytes(data[entry_offset + 12..entry_offset + 16].try_into().unwrap()), // reserve
                    u64::from_be_bytes(data[entry_offset + 16..entry_offset + 24].try_into().unwrap()), // timestamp
                ),
        ...
        RdtaBox { base, data_table }
    }
}

impl Serialize for RdtaBox {
...
mod tests {
    ...
    fn test_rdtabox_read() {
        ...
        let rdta_box = RdtaBox::read(&data);
        assert_eq!(rdta_box.data_table.len(), 2);

        assert_eq!(rdta_box.data_table[0].x, 10.0);
        assert_eq!(rdta_box.data_table[0].y, 12.0);
        assert_eq!(rdta_box.data_table[0].z, 14.0);
        assert_eq!(rdta_box.data_table[0].timestamp, 1);

        assert_eq!(rdta_box.data_table[1].x, 20.0);
        assert_eq!(rdta_box.data_table[1].y, 22.0);
        assert_eq!(rdta_box.data_table[1].z, 24.0);
        assert_eq!(rdta_box.data_table[1].timestamp, 2);
    }
    
    #[test]
    fn test_rdta_box_to_json() {
        let data: Vec<u8> = setup();
        let rdta_box = RdtaBox::read(&data);
        let json_output = serde_json::to_string_pretty(&rdta_box).unwrap();
